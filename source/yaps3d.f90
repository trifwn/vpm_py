...
contains
    module subroutine yaps3d(DSOL_pm, DRHS_pm, Xbound_bl, Xbound_coarse, Dpm_fine, Dpm_coarse, NNbl, NNbl_bl, &
        ...
        ! Write the information of the Yaps Solver to the console.
        ! Mainly: NN_coarse, NNbl, ND, BLOCKS, ibtyp, neqs, neqf, iyntree, ilevmax, npmsize
        if (my_rank .eq. 0) then
            write (dummy_string, "(A)") 'YAPS3D Solver Settings'
            call vpm_print(dummy_string, yellow, 2)
            write (dummy_string, "(A,3I4)") achar(9)//'NN coarse=', NN_coarse(1), NN_coarse(2), NN_coarse(3)
            call vpm_print(dummy_string, nocolor, 2)
            do rank = 0, np - 1
                write (dummy_string, "(A,I3,A,3I4)") &
                    achar(9)//"BLOCK", rank, ' -> NN_bl=', NNbl(1, nb), NNbl(2, nb), NNbl(3, nb)
                call vpm_print(dummy_string, nocolor, 2)
            end do
            write (dummy_string, "(A,I2)") achar(9)//'ND=', ND
            call vpm_print(dummy_string, nocolor, 2)
            write (dummy_string, "(A,I2)") achar(9)//'BLOCKS=', BLOCKS
            call vpm_print(dummy_string, nocolor, 2)
            write (dummy_string, "(A,I2)") achar(9)//'ibctyp=', ibctyp
            call vpm_print(dummy_string, nocolor, 2)
            write (dummy_string, "(A,I2,A,I2)") achar(9)//'neqs=', neqs, ' neqf=', neqf
            call vpm_print(dummy_string, nocolor, 2)
            write (dummy_string, "(A,I2)") achar(9)//'iyntree=', iyntree
            call vpm_print(dummy_string, nocolor, 2)
            write (dummy_string, "(A,I2)") achar(9)//'ilevmax=', ilevmax
            call vpm_print(dummy_string, nocolor, 2)
        end if
